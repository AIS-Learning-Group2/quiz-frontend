...
import {fetchQuiz} from "../../utils/QuizService"
...
const Quiz = () =>{
	...
	const fetchQuizData = async() => {
		if(selectedNumOfQuestion && selectedSubject){
			const questions =  await fetchQuiz(selectedNumOfQuestion, selectedSubject)
			setQuizQuestions(questions)
		}
	}

	const handleAnswerChange = (questionId, option) =>{
		setSelectedAnswers((prevOption) => {
			const existingOptionIndex = prevOption.findIndex((optionObj) => optionObj.id === questionId)
			const selectedAnswer = Array.isArray(option) 
			? option.map((a) => a.charAt(0)) 
			: option.charAt(0)

			if(existingOptionIndex  !== -1){
				const updatedAnswers = [...prevOption]
				updatedAnswers[existingOptionIndex] = {id: questionId, option: selectedAnswer}
				return updatedAnswers
			}else{
				const newAnswer = {id : questionId, answer : selectedAnswer}
				return [...prevOption, newAnswer]
			}
		})
	}
	...
	const isChecked =(questionId, option) =>{
		...
		if(Array.isArray(selectedAnswer.answer)){
			return selectedAnswer.answer.includes(multiple_option.charAt(0))
		}
		return selectedAnswer.answer === option.At(0)
	}

	const handleCheckeboxChange = (questionId, option) =>{
		setSelectedAnswers((prevOption) => {
			const existingOptionIndex = prevOption.findIndex((optionObj) => optionObj.id === questionId)
			const selectedAnswer = Array.isArray(option) 
			? option.map((a) => a.charAt(0)) 
			...
			if(existingOptionIndex !== -1){
				const updatedAnswers =  [...prevOption]
				const existingAnswers =  updatedAnswers[existingOptionIndex].answer
				let newAnswer
				...
				updatedAnswers[existingOptionIndex] = {id : questionId, answer: newAnswer}
				return updatedAnswers
			}else{
				const newAnswer = {id : questionId , answer: [selectedAnswer]}
				return [...prevOption, newAnswer]
...
const handleSubmit = () =>{
	...
	quizQuestions.forEach((question) => {
		...
		if(selectedAnswer){
			...
			const correctOptions = Array.isArray(question.correct_option)
			? question.correct_option : [question.correct_option]
			const isCorrect = selectedOptions.every((op) => 
			correctOptions.includes(op))
...
const handlePreviousQuestion =()=>{
	...
	return (
		<div className="p-5">
			...
			handleCheckboxChange={handleCheckeboxChange} />
